...
const password = Joi.string().min(8).max(BCRYPT_MAX_BYTES, 'utf8').required().regex(/^(?=.*?[\p{Lu}])(?=.*?[\p{Ll}])(?=.*?\d).*$/u).message('"{#label}" must contain one uppercase letter, one lowercase letter and one digit.')
